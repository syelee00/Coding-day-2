...
type IllustratedRecommendation = Recommendation & { imageUrl?: string };
...
export default function Home() {
  ...
  const [results, setResults] = useState<IllustratedRecommendation[] | null>(null);
  ...
  async function recommend(event: FormEvent) {
    ...
    try {
      ...
      const recommendations: IllustratedRecommendation[] = data.recommendations;
      setResults(recommendations);
      void Promise.all(recommendations.map(async (item) => {
        const illustration = await fetch("/api/illustrate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: item.name }) });
        if (!illustration.ok) return;
        const { imageUrl } = await illustration.json();
        setResults((current) => current?.map((recommendation) => recommendation.name === item.name ? { ...recommendation, imageUrl } : recommendation) ?? null);
      }));
    } catch (caught) { setError(caught instanceof Error ? caught.message : "추천을 불러오지 못했어요."); }
    finally { setLoading(false); }
  }

  if (results) return <main className="shell result-shell"><button className="back" onClick={() => setResults(null)}>← 조건 수정하기</button><header><p className="eyebrow">TODAY&apos;S PICK</p><h1>오늘은 이 메뉴 어때요?</h1><p>당신의 조건을 반영한 세 가지 추천이에요.</p></header><section className="result-grid">{results.map((item, index) => <article className="recommendation-card" key={item.name}><span className="number">0{index + 1}</span><div className="illustration">{item.imageUrl ? <img src={item.imageUrl} alt={`${item.name} 일러스트`} /> : <span>일러스트를 그리고 있어요…</span>}</div><span className="category">{item.category}</span><h2>{item.name}</h2><p>{item.reason}</p><div className="drink"><span>어울리는 음료</span><strong>{item.drink}</strong></div></article>)}</section><button className="primary-button retry" onClick={() => setResults(null)}>다른 메뉴 추천받기</button></main>;
